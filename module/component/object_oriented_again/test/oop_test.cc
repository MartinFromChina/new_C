...
W_CLASS_DEF(stack
	,// public
		validator   check_method; // also could be a struct  ,with many parameters and funcs
		uint16_t cur_size;
	    uint32_t buf[MAX_STACK_SIZE];
	////////,// private
	 
);

NEW_W_CLASS_PUBLIC(stack,stack1){
	"stack1",
	NO_CONSTRUCTOR,
	NO_DESTRUCTOR,
	(validator)0,
...
X_Boolean PushMethod(uint32_t value,s_all_class_base const * p_base)
{
	if(p_base == (s_all_class_base *)0)  {return X_False;}
	stack * p_common = (stack *)p_base;

	// to do later : check the obj p_base type , only stack type can be used 

	if(p_common ->cur_size >= MAX_STACK_SIZE) {return X_False;}
	
	if(p_common ->check_method != (validator)0)
	{
		if(p_common ->check_method(value) != X_True) {return X_False;}
	}

	p_common ->buf[p_common ->cur_size ++] = value;
	return X_True;
}
X_Boolean PopMethod(uint32_t *p_value,s_all_class_base const * p_base)
{
	if(p_base == (s_all_class_base *)0 || p_value == (uint32_t *)0)  {return X_False;}
	stack * p_common = (stack *)p_base;

	// to do later : check the obj p_base type , only stack type can be used 

	if(p_common ->cur_size == 0) {return X_False;}
	if(p_common ->cur_size > MAX_STACK_SIZE) // impossible
	{
		p_common ->cur_size = MAX_STACK_SIZE;
	}
	*p_value = p_common ->buf[--p_common ->cur_size];
	return X_True;
}

X_Boolean Range_Validator(uint32_t value)
{
	if(value < 10000 && value > 500) {return X_True;}
	return X_False;
}

NEW_W_CLASS_PUBLIC(stack,stack2){
	"stack1",
	NO_CONSTRUCTOR,
	NO_DESTRUCTOR,
	Range_Validator,
...
X_Boolean PreviousValidator(uint32_t value)
{
	if(value > pre_value )
	{
		pre_value = value;
...
NEW_W_CLASS_PUBLIC(stack,stack3){
	"stack1",
	NO_CONSTRUCTOR,
	NO_DESTRUCTOR,
	PreviousValidator,
...
TEST(stack,mul_check)
{
	X_Boolean isOK;
	uint32_t t_value;
	isOK = PushMethod(100,&stack1.base);EXPECT_EQ(isOK,X_True);
	isOK = PushMethod(5,&stack1.base);EXPECT_EQ(isOK,X_True);
	isOK = PushMethod(38,&stack1.base);EXPECT_EQ(isOK,X_True);
	isOK = PushMethod(99999,&stack1.base);EXPECT_EQ(isOK,X_True);
	isOK = PushMethod(6543,&stack1.base);EXPECT_EQ(isOK,X_True);

	isOK = PopMethod(&t_value,&stack1.base);  EXPECT_EQ(isOK,X_True); EXPECT_EQ(t_value,6543);
	isOK = PopMethod(&t_value,&stack1.base);  EXPECT_EQ(isOK,X_True); EXPECT_EQ(t_value,99999);
	isOK = PopMethod(&t_value,&stack1.base);  EXPECT_EQ(isOK,X_True); EXPECT_EQ(t_value,38);
	isOK = PopMethod(&t_value,&stack1.base);  EXPECT_EQ(isOK,X_True); EXPECT_EQ(t_value,5);
	isOK = PopMethod(&t_value,&stack1.base);  EXPECT_EQ(isOK,X_True); EXPECT_EQ(t_value,100);
	isOK = PopMethod(&t_value,&stack1.base);  EXPECT_EQ(isOK,X_False); 

	isOK = PushMethod(6543,&stack2.base);EXPECT_EQ(isOK,X_True);
	isOK = PushMethod(3,&stack2.base);EXPECT_EQ(isOK,X_False);
	isOK = PushMethod(20000,&stack2.base);EXPECT_EQ(isOK,X_False);

	isOK = PushMethod(111,&stack3.base);EXPECT_EQ(isOK,X_True);
	isOK = PushMethod(222,&stack3.base);EXPECT_EQ(isOK,X_True);
	isOK = PushMethod(333,&stack3.base);EXPECT_EQ(isOK,X_True);
	isOK = PushMethod(256,&stack3.base);EXPECT_EQ(isOK,X_False);
	isOK = PopMethod(&t_value,&stack3.base);  EXPECT_EQ(isOK,X_True); EXPECT_EQ(t_value,333);
	isOK = PopMethod(&t_value,&stack3.base);  EXPECT_EQ(isOK,X_True); EXPECT_EQ(t_value,222);
	isOK = PopMethod(&t_value,&stack3.base);  EXPECT_EQ(isOK,X_True); EXPECT_EQ(t_value,111);
	isOK = PopMethod(&t_value,&stack3.base);  EXPECT_EQ(isOK,X_False); 
