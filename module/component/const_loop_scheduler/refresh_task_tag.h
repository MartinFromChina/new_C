...
#define TASK_BODY_BEGIN(task_func_name,task_init_func,max_if_nest_deepth,max_loop_nest_deepth)             \
                        ...
                        X_Void task_func_name(uint32_t x_task_param_ms,uint16_t task_id,uint8_t *p_isInitDone) {                                         \
                            uint16_t nest_cnt = 0,loop_nest_cnt = 0;   uint32_t loop_line_num_backup = 0;                       \
                            if(p_isInitDone == (uint8_t *)0)    {                                                                        \
                            X_TASK_LOG(1,(" ---- task(id %d) fatal error occur , task parameter  : p_isInitDone is NULL !!!\r\n",task_id)); return; }      \
                            if(*p_isInitDone == 0)                                          \
                            {                                                                                  \
                                *p_isInitDone = 1;                                          \
                                ...
                                if(CONCAT_2(CUR_TASK_TAG,init_func) != (x_one_step)0) {                               \
                                    CONCAT_2(CUR_TASK_TAG,init_func)(x_task_param_ms);                             \
                                }                                                                     \
